// This is a copy of crashpad_wer_main.cc but with explicit dllspec
// Zig doesn't seem to have a way to use the def file as an export source at the
// moment So we use this as a workaround
...
__declspec(dllexport) HRESULT OutOfProcessExceptionEventCallback(
    ...
    BOOL *pbOwnershipClaimed, PWSTR pwszEventName, PDWORD pchSize,
  ...
  *pbOwnershipClaimed = FALSE;
  bool result = crashpad::wer::ExceptionEvent(wanted_exceptions,
                                              sizeof(wanted_exceptions) /
                                                  sizeof(wanted_exceptions[0]),
