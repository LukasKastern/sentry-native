...
std::string CurlErrorMessage(CURLcode curl_err, const std::string &base) {
  return base::StringPrintf("%s: %s (%d)", base.c_str(),
                            Libcurl::CurlEasyStrError(curl_err), curl_err);
}
...
private:
  static size_t ReadRequestBody(char *buffer, size_t size, size_t nitems,
                                void *userdata);
  static size_t WriteResponseBody(char *buffer, size_t size, size_t nitems,
                                  void *userdata);
};
...
#define TRY_CURL_SLIST_APPEND(slist, data)                                     \
  ...
  if (!root_ca_certificate_path().empty()) {
    TRY_CURL_EASY_SETOPT(curl.get(), CURLOPT_CAINFO,
                         root_ca_certificate_path().value().c_str());
  }

  constexpr int kMillisecondsPerSecond = 1E3;
  TRY_CURL_EASY_SETOPT(curl.get(), CURLOPT_TIMEOUT_MS,
  ...
  if (method() == "POST") {
    ...
    if (chunked) {
      ...
      TRY_CURL_EASY_SETOPT(curl.get(), CURLOPT_POSTFIELDSIZE_LARGE,
                           content_length_curl);
    }
  } else if (method() != "GET") {
    // Untested.
...
#undef TRY_CURL_SLIST_APPEND
  ...
  if (status != 200) {
    LOG(ERROR) << base::StringPrintf("HTTP status %ld, response = \"%s\"",
...
// static
size_t HTTPTransportLibcurl::ReadRequestBody(char *buffer, size_t size,
  ...
  len = std::min(len, static_cast<size_t>(
                          std::numeric_limits<FileOperationResult>::max()));
...
// static
size_t HTTPTransportLibcurl::WriteResponseBody(char *buffer, size_t size,
                                               size_t nitems, void *userdata) {
